...
class ConversationManager:
    ...
    def ensure_tool_result_pairing(
        self,
        reason: str = "Error: tool call was interrupted.",
    ) -> int:
        """Make every tool_use have exactly one tool_result. Returns repairs made."""
        ...
        for message in self.history:
            if message.role == "assistant" and message.tool_uses:
                unique_uses: list[ToolUseBlock] = []
                for tool_use in message.tool_uses:
                    if tool_use.tool_id in seen_uses:
                        repaired += 1
                        continue
                    seen_uses.add(tool_use.tool_id)
                    unique_uses.append(tool_use)
                    pending.append(tool_use.tool_id)
                cleaned.append(
                    Message(
                        role="assistant",
                        content=message.content,
                        tool_uses=unique_uses,
            ...
            if message.tool_results:
                kept: list[ToolResultBlock] = []
                for result in message.tool_results:
                    if result.tool_use_id in pending:
                        pending.remove(result.tool_use_id)
                        kept.append(result)
                    else:
                        repaired += 1
                if kept or message.content:
                    cleaned.append(
                        Message(
                            role="user",
                            content=message.content,
        ...
        if pending:
            self.add_tool_results_message(
                [ToolResultBlock(tool_id, reason, is_error=True) for tool_id in pending]
            )
