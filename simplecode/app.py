...
from simplecode.agents import AgentLoader, TaskManager, TraceRegistry, inject_task_notifications
from simplecode.agents.tool_filter import build_teammate_tools
from simplecode.cache import FileCache
from simplecode.client import LLMError, create_client
from simplecode.commands import CommandContext, CommandRegistry, complete, parse_command
from simplecode.commands.completion import CompletionPopup
from simplecode.commands.handlers import (
    TEAM_COMMAND,
    WORKTREE_COMMAND,
...
from simplecode.context import ensure_session_dir, estimate_conversation_tokens
from simplecode.conversation import ConversationManager, Message
from simplecode.evidence import EvidenceBundle
from simplecode.hooks import HookContext, HookEngine, load_hooks
from simplecode.mcp import MCPManager
from simplecode.memory import MemoryManager, SessionManager, load_instructions
from simplecode.permission_dialog import InlinePermissionPrompt, InlineQuestionPrompt
...
from simplecode.skills import SkillExecutor, SkillLoader, validate_skill_dependencies
from simplecode.teams import TeamManager
from simplecode.teams.transcript import load_transcript
from simplecode.tools import ToolRegistry, create_default_registry, register_task_tools
from simplecode.tools.agent_tool import AgentTool
from simplecode.tools.ask_user import AskUserEvent, AskUserTool
from simplecode.tools.impl import ToolSearchTool
...
from simplecode.tools.team_create import TEAM_RUNTIME_TOOLS, TeamCreateTool
from simplecode.tools.team_delete import TeamDeleteTool
from simplecode.tools.team_merge import TeamMergeTool
from simplecode.tools.team_stop import TeamStopTool
from simplecode.worktree import (
    EnterWorktreeTool,
    ExitWorktreeTool,
    WorktreeManager,
...
class SimpleCodeApp(App[None]):
    ...
    def __init__(
        ...
        *,
        ...
        if team_name:
            ...
            teammate_name = os.getenv("SIMPLECODE_TEAMMATE_NAME", "")
        ...
        self._pending_answer_widget: Static | None = None
        self._pending_answer_text = ""
        self._pending_answer_dirty = False
        self._pending_thinking_widget: Static | None = None
        self._pending_thinking_text = ""
        self._pending_thinking_dirty = False
    ...
    def _set_active_work_dir(self, work_dir: Path) -> None:
        ...
        self.permission_checker.rule_engine = RuleEngine(
            user_rules_path=Path.home() / ".simplecode" / "permissions.yaml",
            project_rules_path=active / ".simplecode" / "permissions.yaml",
            local_rules_path=active / ".simplecode" / "permissions.local.yaml",
        )

    def compose(self) -> ComposeResult:
        with VerticalScroll(id="conversation"):
            with Vertical(id="welcome-panel") as welcome_panel:
                welcome_panel.border_title = f" Simple Code v{__version__} "
                ...
                with Horizontal(id="welcome-content"):
                    with Vertical(id="welcome-left"):
                        ...
                        yield Static(f"Simple Code v{__version__}", id="product-line", markup=False)
        ...
        try:
            ...
            async for event in self.agent.run(self.conversation):
                ...
                if isinstance(event, StreamText):
                    ...
                    self._queue_live_answer(answer_widget, "".join(answer_parts))
                    now = perf_counter()
                    ...
                    if event.complete:
                        if thinking_parts:
                            self._queue_live_thinking(
                                thinking_details, "".join(thinking_parts)
                            )
                            self._flush_live_thinking()
                        ...
                        self._queue_live_thinking(thinking_details, "".join(thinking_parts))
                        now = perf_counter()
                    ...
                    # Tools and turn events can take a long time. Push the last
                    # buffered tokens before we stop receiving StreamText.
                    self._flush_live_answer()
                    self._flush_live_thinking()
            ...
            self._pending_answer_widget = None
            self._pending_answer_text = ""
            self._pending_answer_dirty = False
            self._pending_thinking_widget = None
            self._pending_thinking_text = ""
            self._pending_thinking_dirty = False
    ...
    async def _render_markdown(
        ...
        if not text:
            return
        self._flush_live_answer()
        if self._pending_answer_widget is plain_widget:
            self._pending_answer_widget = None
            self._pending_answer_text = ""
            self._pending_answer_dirty = False
    ...
    def _queue_live_answer(self, widget: Static, text: str) -> None:
        self._pending_answer_widget = widget
        self._pending_answer_text = text
        self._pending_answer_dirty = True

    def _queue_live_thinking(self, widget: Static, text: str) -> None:
        self._pending_thinking_widget = widget
        self._pending_thinking_text = text
        self._pending_thinking_dirty = True

    def _flush_live_answer(self) -> None:
        if not self._pending_answer_dirty:
            return
        widget = self._pending_answer_widget
        if widget is not None and widget.is_mounted:
            widget.update(terminal_safe_text(self._pending_answer_text))
        self._pending_answer_dirty = False

    def _flush_live_thinking(self) -> None:
        if not self._pending_thinking_dirty:
            return
        widget = self._pending_thinking_widget
        if widget is not None and widget.is_mounted:
            widget.update(terminal_safe_text(self._pending_thinking_text))
        self._pending_thinking_dirty = False
    ...
    async def start_headless(self) -> None:
        ...
        self.agent.allow_permission_prompts = False
