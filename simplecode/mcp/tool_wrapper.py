"""MCP tool wrapper: adapts MCP tools to the Simple Code Tool base class."""
...
def _normalize_tool_name(name: str) -> str:
    """Map an MCP tool name to the provider-safe Simple Code tool namespace."""
...
def _annotation_flag(tool_def: types.Tool, name: str) -> bool | None:
...
def _meta_mapping(tool_def: types.Tool) -> dict[str, Any]:
    meta = getattr(tool_def, "meta", None) or getattr(tool_def, "_meta", None) or {}
...
class MCPToolWrapper(Tool):
    """Adapts a single MCP tool definition to the Simple Code Tool interface."""
    ...
    def __init__(
        ...
        *,
        ...
        read_only = _annotation_flag(tool_def, "readOnlyHint")
        destructive = _annotation_flag(tool_def, "destructiveHint")
        meta = _meta_mapping(tool_def)
        always_load = bool(meta.get("anthropic/alwaysLoad") or meta.get("alwaysLoad"))

        # Override ClassVar attributes with instance values (allowed in Python)
        self.name = wrapper_name  # type: ignore[misc]
        self.description = _truncate_description(  # type: ignore[misc]
            tool_def.description or f"MCP tool {wrapper_name}"
        )
        if read_only is True:
            self.category = "read"  # type: ignore[misc]
            self.is_concurrency_safe = True  # type: ignore[misc]
            self.is_plan_safe = True  # type: ignore[misc]
            self.is_destructive = False  # type: ignore[misc]
        else:
            ...
            self.is_destructive = destructive is True  # type: ignore[misc]
        self.should_defer = not always_load  # type: ignore[misc]
