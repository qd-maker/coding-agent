...
from simplecode.agents.parser import AgentDefinition
from simplecode.agents.task_manager import BackgroundTask, TaskManager
from simplecode.agents.tool_filter import build_teammate_tools, resolve_agent_tools
from simplecode.agents.trace import TraceNode, TraceRegistry
from simplecode.client import LLMClient, create_client
...
from simplecode.teams.models import BackendType, TeammateInfo, sanitize_name
...
class _NonInteractivePermissionChecker(PermissionChecker):
    ...
    def check(self, tool: Tool, arguments: dict[str, Any]) -> Decision:
        ...
        if decision.effect == "ask":
            return Decision(
                "deny",
                "Non-interactive sub-agent cannot request confirmation",
                "system",
            )
        return decision
    ...
    def _permission_checker(
        ...
        return _NonInteractivePermissionChecker(
            ...
            rule_engine=RuleEngine(
                ...
                project_rules_path=work_dir / ".simplecode" / "permissions.yaml",
                local_rules_path=work_dir / ".simplecode" / "permissions.local.yaml",
            ),
            mode=mode,
        )
