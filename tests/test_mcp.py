...
from simplecode.mcp.tool_wrapper import MCPToolWrapper, _extract_text
...
class TestMCPToolWrapper:
    ...
    def test_readonly_annotation_maps_to_read_category(self) -> None:
        from mcp import types
        ...
        wrapper = MCPToolWrapper(manager=mgr, server_name="docs", tool_def=tool_def)
        assert wrapper.category == "read"
        assert wrapper.is_concurrency_safe is True
        assert wrapper.is_plan_safe is True
        assert wrapper.is_destructive is False
        assert len(wrapper.description) == 2048
        assert wrapper.description.endswith("…")

    def test_destructive_annotation_stays_command(self) -> None:
        from mcp import types

        mgr = _make_manager("ops")
        tool_def = types.Tool(
            name="wipe",
            description="danger",
            inputSchema={"type": "object", "properties": {}},
        ...
        wrapper = MCPToolWrapper(manager=mgr, server_name="ops", tool_def=tool_def)
...
class TestMCPManagerPartialFailure:
    ...
    async def test_slow_servers_timeout_concurrently(self) -> None:
        ...
        with patch("simplecode.mcp.manager.MCPClient", SlowClient):
    ...
    async def test_one_failure_does_not_block_others(self) -> None:
        ...
        with patch("simplecode.mcp.manager.MCPClient", FakeClient):
...
async def test_lazy_registration_is_idempotent_for_concurrent_callers(
    ...
    import simplecode.mcp.manager as manager_module
