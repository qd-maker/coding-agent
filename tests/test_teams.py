...
from simplecode.teams.coordinator import get_coordinator_system_prompt, is_coordinator_mode
from simplecode.teams.mailbox import Mailbox, create_message
from simplecode.teams.manager import TeamError, TeamManager
...
from simplecode.teams.spawn_inprocess import spawn_inprocess_teammate
...
from simplecode.tools.base import StreamEnd, StreamEvent, TextDelta, ToolResult
from simplecode.tools.send_message import SendMessageParams, SendMessageTool
...
def test_backend_detection_explicit_and_noninteractive(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("simplecode.teams.backend_detect.shutil.which", lambda _name: None)
...
def test_coordinator_requires_double_lock_and_has_four_stages(
    ...
    monkeypatch.setenv("SIMPLECODE_COORDINATOR_MODE", "1")
...
def test_build_cli_command_quotes_prompt_and_carries_team_env(tmp_path: Path) -> None:
    ...
    assert "SIMPLECODE_TEAM_NAME=demo" in command
