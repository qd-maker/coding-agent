...
from simplecode.permissions import PathSandbox, PermissionChecker, PermissionMode, RuleEngine
...
class TestHttpExecutor:
    ...
    async def test_mock_request_expands_json_and_limits_response(self) -> None:
        ...
        with patch("simplecode.hooks.executors.urlopen", fake_urlopen):
...
class TestHookEngine:
    ...
    async def test_action_exception_is_isolated(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr("simplecode.hooks.engine.execute_action", broken)
    ...
    async def test_async_hook_returns_without_waiting(
        ...
        monkeypatch.setattr("simplecode.hooks.engine.execute_action", delayed)
        engine = HookEngine(
