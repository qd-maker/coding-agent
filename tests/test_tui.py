...
import simplecode.app as app_module
from simplecode.app import ActivityLine, SimpleCodeApp, PromptComposer, terminal_safe_text
from simplecode.client import LLMClient
from simplecode.commands.completion import CompletionPopup
from simplecode.config import AppConfig, MCPServerConfig
from simplecode.conversation import ConversationManager, Message
from simplecode.memory import SessionManager
...
from simplecode.skills import SkillDependencyError
...
def isolate_tui_runtime(
    ...
    monkeypatch.setattr(
        "simplecode.skills.loader.USER_SKILLS_DIR",
        tmp_path / "isolated-user-skills",
    )
    monkeypatch.setattr(
        "simplecode.agents.loader.USER_AGENTS_DIR",
...
async def test_tui_shell_fits_80_by_24(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    app = SimpleCodeApp(config)

    async with app.run_test(size=(80, 24)) as pilot:
        await pilot.pause()
        assert "Simple Code" in str(app.query_one("#product-line", Static).render())
...
async def test_tui_slash_commands_are_intercepted_before_agent(
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        assert "Simple Code 状态" in str(app.query(".tool").last(Static).render())
...
async def test_ch11_tui_hot_reloads_project_skill(
    ...
    skill_path = tmp_path / ".simplecode" / "skills" / "custom.md"
...
def test_ch11_app_fails_fast_for_missing_skill_tool(
    ...
    skill_path = tmp_path / ".simplecode" / "skills" / "broken-dependency.md"
    skill_path.parent.mkdir(parents=True)
    skill_path.write_text(
...
class TwoChunkHangClient(LLMClient):
    def __init__(self, config: Any) -> None:
        super().__init__(config)
        self.started = asyncio.Event()
        self.release = asyncio.Event()
    ...
    async def stream(
        ...
        del conversation, system, tools
        yield TextDelta("I'll create a complete Snake game in a single HTML file with a ")
        await asyncio.sleep(0)
        yield TextDelta("canvas and keyboard controls.")
        self.started.set()
        await self.release.wait()
        yield StreamEnd("end_turn", 4, 8)


@pytest.mark.asyncio
async def test_streamed_tail_is_visible_before_completion(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    config = app_config()
    client = TwoChunkHangClient(config.provider)
    monkeypatch.setattr(app_module, "create_client", lambda _: client)
    app = SimpleCodeApp(config)

    async with app.run_test(size=(100, 30)) as pilot:
        await pilot.press(*list("snake"), "enter")
        await asyncio.wait_for(client.started.wait(), timeout=1)
        await pilot.pause(0.15)

        answer = app.query_one(".assistant", Static)
        visible = str(answer.render())
        assert "I'll create a complete Snake game" in visible
        assert "canvas and keyboard controls" in visible

        client.release.set()
        await pilot.pause(0.2)
        markdown = app.query_one(".assistant-markdown", Markdown)
        assert "canvas and keyboard controls" in markdown.source
...
async def test_tui_executes_read_file_and_returns_result(
    ...
    target.write_text("# Simple Code", encoding="utf-8")
    config = app_config()
    ...
    async with app.run_test(size=(100, 30)) as pilot:
        ...
        assert all("# Simple Code" not in line for line in tool_lines)
...
async def test_ch9_tui_loads_instructions_persists_and_exposes_commands(
    ...
    monkeypatch.chdir(tmp_path)
    (tmp_path / "SIMPLECODE.md").write_text("Always run tests.", encoding="utf-8")
    config = app_config()
    client = StreamingClient(config.provider)
    monkeypatch.setattr(app_module, "create_client", lambda _: client)
    app = SimpleCodeApp(config)
    ...
    jsonl = tmp_path / ".simplecode" / "sessions" / f"{session_id}.jsonl"
