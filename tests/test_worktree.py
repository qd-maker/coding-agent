...
from simplecode.commands.handlers.worktree import handle_worktree
...
def git_repo(tmp_path: Path) -> Path:
    ...
    root.mkdir()
    _git(root, "init")
    _git(root, "config", "user.email", "simplecode@example.test")
    _git(root, "config", "user.name", "Simple Code Test")
...
async def test_fast_recovery_does_not_spawn_git(
    ...
    monkeypatch.setattr("simplecode.worktree.manager.run_git", fail_git)
...
async def test_subagent_worktree_preserves_changes_and_parent(git_repo: Path) -> None:
    ...
    assert (git_repo / "witness.txt").read_text(encoding="utf-8") == "main"
    preserved = next((git_repo / ".simplecode/worktrees").glob("agent-*"))
...
async def test_subagent_clean_worktree_is_removed(git_repo: Path) -> None:
    ...
    assert list((git_repo / ".simplecode/worktrees").glob("agent-*")) == []
