...
from mewcode.commands import CommandContext
...
from mewcode.memory.session import RecordType, records_to_messages
...
class TestLoadInstructions:
    def test_single_layer(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        project.mkdir()
        (project / "MEWCODE.md").write_text("project", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
        ...
        (project / ".mewcode" / "MEWCODE.md").write_text("local", encoding="utf-8")
...
class TestMemoryManager:
    ...
    def test_load_merges_user_and_project(
        ...
        (home / ".mewcode").mkdir()
        (project / ".mewcode").mkdir(parents=True)
        (home / ".mewcode" / "memories.md").write_text("user", encoding="utf-8")
        (project / ".mewcode" / "memories.md").write_text("project", encoding="utf-8")
