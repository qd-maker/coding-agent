...
from simplecode.agent import Agent, LoopComplete
from simplecode.client import LLMClient
from simplecode.commands import CommandContext, CommandRegistry
...
from simplecode.tools.base import StreamEnd, StreamEvent, TextDelta, ToolCallComplete, ToolResult
...
def isolate_user_skills(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr("simplecode.skills.loader.USER_SKILLS_DIR", user_dir)
    return user_dir
...
def test_loader_precedence_directory_layout_and_source_labels(
    ...
    write_skill(
        project / ".simplecode" / "skills" / "review" / "SKILL.md",
        name="review",
...
def test_loader_skips_bad_file_and_hot_reload_falls_back_to_cache(
    ...
    isolate_user_skills(monkeypatch, tmp_path)
    root = tmp_path / "project" / ".simplecode" / "skills"
    good = root / "custom.md"
...
async def test_load_directory_skill_registers_specialized_tool(
    ...
    isolate_user_skills(monkeypatch, tmp_path)
    root = tmp_path / ".simplecode" / "skills" / "bundle"
