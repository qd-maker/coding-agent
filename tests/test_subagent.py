...
from simplecode.agents.notification import (
...
from simplecode.commands.handlers.tasks import handle_task, handle_tasks, handle_trace
from simplecode.commands.registry import CommandContext
from simplecode.config import ProviderConfig
from simplecode.conversation import ConversationManager, ThinkingBlock, ToolUseBlock
...
from simplecode.tools.base import StreamEnd, StreamEvent, TextDelta, Tool, ToolResult
...
class TestAgentLoader:
    ...
    def test_project_overrides_user_builtin_and_plugin(
        ...
        _write_agent(home / ".simplecode/agents/plan.md", name="Plan")
        plugin = tmp_path / "plugin"
        _write_agent(plugin / "plan.md", name="Plan")
        project = _write_agent(tmp_path / ".simplecode/agents/plan.md", name="Plan")
    ...
    def test_hot_reload_and_invalid_fallback(self, tmp_path: Path) -> None:
        path = _write_agent(tmp_path / ".simplecode/agents/hot.md", name="hot")
        loader = AgentLoader(tmp_path)
        loader.load_all()
        path.write_text(
