...
PROJECT_AGENTS_DIR = Path(".mewcode/agents")
USER_AGENTS_DIR = Path("~/.mewcode/agents")
...
class AgentLoader:
    ...
    def __init__(
        ...
        *,
        ...
        self._builtin_dir = Path(str(resources.files("mewcode.agents.builtins"))).resolve()
        discovered_plugins = [
            entry / "agents"
            for plugin_root in (
                self.work_dir / ".mewcode" / "plugins",
                Path.home() / ".mewcode" / "plugins",
            )
