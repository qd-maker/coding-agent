...
`~/.simplecode/teams/<team>/`，应用退出只停止本地协程，不删除团队和成员上下文。
...
- `in-process`：同进程协程，LLM/Hook/文件系统基础设施共享；Conversation、权限记录、Token 与
  文件工具实例隔离。
- `tmux` / `iTerm2`：通过 `simplecode -p --work-dir ...` 启动完整独立 CLI；团队名、成员名和 mailbox
...
`SIMPLECODE_COORDINATOR_MODE=1|true|yes` 同时满足才激活。激活后 Lead 不拥有 Read/Glob/Grep、
