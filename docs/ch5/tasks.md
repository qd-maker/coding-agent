...
- 影响文件: `simplecode/prompts.py:10-28`
- 依赖任务: 无
- 完成标准: `@dataclass class PromptSection` 含 `name: str / priority: int / content: str`（`simplecode/prompts.py:10-15`）；`PromptBuilder.__init__` 维护 `self._sections: list[PromptSection]`（`simplecode/prompts.py:17-19`）；`PromptBuilder.add(section)` 返回 `PromptBuilder` 自身支持链式调用（`simplecode/prompts.py:21-23`）。

## T2: 实现 PromptBuilder.build 排序 + 拼接
- 影响文件: `simplecode/prompts.py:25-28`
- 依赖任务: T1
- 完成标准: `build()` 用 `self._sections.sort(key=lambda s: s.priority)` 按 priority 升序排（`simplecode/prompts.py:26`）；trim 后空 content 不进入 parts；用 `"\n\n".join(parts)` 输出最终文本（`simplecode/prompts.py:27-28`）。

## T3: 实现 environment_section 工厂函数
- 影响文件: `simplecode/prompts.py:147-154`
- 依赖任务: T1
...
- 影响文件: `simplecode/prompts.py:35-145`
- 依赖任务: T1
- 完成标准:
 - `IDENTITY_SECTION`（priority=0，`prompts.py:35-48`）—— Simple Code 身份 + 安全 / URL 不乱造
...
- 影响文件: `simplecode/prompts.py:233-274`
- 依赖任务: T2, T3, T4
- 完成标准: 签名 `build_system_prompt(hook_prompts, coordinator_mode, agent_catalog, custom_instructions, skill_section, memory_section, work_dir)`（`prompts.py:233-241`）；`coordinator_mode=True` 时委托给 `simplecode.teams.coordinator.get_coordinator_system_prompt`（`prompts.py:242-244`）；否则按 Identity→System→DoingTasks→ExecutingActions→UsingTools→ToneStyle→TextOutput→environment_section 顺序 Add 8 个固定 section（`prompts.py:246-254`）；依次按 `custom_instructions`（priority=80） / `skill_section`（priority=90） / `memory_section`（priority=95）按需 Add（`prompts.py:256-267`）；空字符串不 Add；`hook_prompts` 非空时尾部追加 `# Hook Injected Context\n` + `\n`.join（`prompts.py:271-272`）。

## T6: 实现 build_plan_mode_reminder 动态指令
- 影响文件: `simplecode/prompts.py:161-226`
- 依赖任务: 无
...
- 影响文件: `simplecode/prompts.py:277-304`
- 依赖任务: 无
...
- 影响文件:
 - `simplecode/agent.py:399-402` `Agent.run` 启动时调 `build_environment_context` + `conversation.inject_environment`
 - `simplecode/agent.py:469-473` `Agent.run` 每轮迭代调 `build_system_prompt`
 - `simplecode/agent.py:480-484` Plan Mode 下调 `build_plan_mode_reminder` 并 `conversation.add_system_reminder`
 - `simplecode/agent.py:898-901` 自动 compact 后重新注入 environment
 - `simplecode/agent.py:918-921` `run_to_completion` 启动时也注入 environment
 - `simplecode/agent.py:935-938` `run_to_completion` 调 `build_system_prompt`
- 依赖任务: T1~T7
...
- 依赖任务: T8
- 完成标准:
 - `ruff check simplecode/prompts.py` 无 lint 错误。
