...
- [x] `simplecode/worktree/slug.py`：安全校验、扁平化、临时名称生成。
- [x] `simplecode/worktree/models.py`：Worktree、Session、变更与清理结果。
- [x] `simplecode/worktree/session.py`：原子持久化和无 Git 子进程 HEAD 恢复。
- [x] `simplecode/worktree/changes.py`：dirty、新 commit、未推送检查及兼容计数。
- [x] `simplecode/worktree/setup.py`：本地配置、hooks、依赖软链接、ignored include。
- [x] `simplecode/worktree/manager.py`：完整 lifecycle、锁、缓存回调、恢复和保护删除。
- [x] `simplecode/worktree/cleanup.py`：三层过期过滤和周期后台任务。
...
- [x] `simplecode --resume` 恢复崩溃前的活动 worktree。
