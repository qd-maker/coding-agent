...
5. 缓存与上下文切换：进入/退出时清空 `FileCache`，重定向全部核心工具，重新加载
   `SIMPLECODE.md`、项目 memory、权限规则、Agent 环境目录和计划文件缓存。
6. 删除保护：有 staged/unstaged/untracked 文件、新 commit、未推送 commit，或 Git 检查失败时，
   默认拒绝删除；只有显式 `discard` 才允许强制清理。
7. 会话恢复：状态原子写入 `.simplecode/worktree_session.json`；`simplecode --resume` 验证并恢复活动
...
- worktree 统一位于 `<repo>/.simplecode/worktrees`，该目录必须被 Git 忽略。
