...
- 文件：`simplecode/commands/registry.py`、`simplecode/commands/parser.py`
...
- 文件：`simplecode/commands/handlers/`
...
- 文件：`simplecode/permissions/rules.py`
- 产出：`clear_local_rules()` 只清理 local tier，不影响用户级和项目级规则。

## T5：实现补全组件

- 文件：`simplecode/commands/completion.py`
...
- 验收命令：
  ...
  - `ruff check .`
  - `mypy simplecode`
  - `python -m compileall -q simplecode`
