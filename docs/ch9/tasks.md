...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/instructions.py:9-46`（`process_includes`）
- 依赖任务: 无
- 完成标准:
...
## T2: 项目指令三层加载
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/instructions.py:48-66`（`load_instructions`）
- 依赖任务: T1
- 完成标准:
  - 三层优先级顺序：`<root>/SIMPLECODE.md` → `<root>/.simplecode/SIMPLECODE.md` → `~/.simplecode/SIMPLECODE.md`
...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:30-119`（`RecordType / SessionRecord`）
- 依赖任务: 无
- 完成标准:
...
## T4: 记录 ↔ 消息互转 + 链路校验
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:122-222`（`records_to_messages / validate_message_chain`）
- 依赖任务: T3
- 完成标准:
...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:225-307`（`SessionMeta / Session / ResumeResult`）
- 依赖任务: T3
- 完成标准:
...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:384-482`（`_generate_session_id / SessionManager`）
- 依赖任务: T4, T5
- 完成标准:
  - `_generate_session_id()`：`session_<YYYYMMDD_HHMMSS>_<4 字符 a-z0-9>` 格式
  - `SessionManager.__init__(work_dir)`：构造 `<work_dir>/.simplecode/sessions/` 并 `mkdir(parents=True, exist_ok=True)`
...
## T7: 断会话时长提示
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:358-380`（`build_time_gap_message`）+ `TIME_GAP_THRESHOLD` 常量
- 依赖任务: 无
- 完成标准:
...
## T8: 会话摘要生成（可选）
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/session.py:316-355`（`generate_session_summary`）+ `SESSION_SUMMARY_PROMPT`
- 依赖任务: 无
- 完成标准:
...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/auto_memory.py:8-71`（常量 + `__init__ / user_path / project_path / load`）
- 依赖任务: 无
- 完成标准:
  - 常量：`USER_MEMORIES_RELPATH = ".simplecode/memories.md"` / `PROJECT_MEMORIES_RELPATH = ".simplecode/memories.md"`
...
- 影响文件: `/Users/codemelo/simplecode/simplecode/memory/auto_memory.py:11-37, 72-127`（`MEMORY_EXTRACTION_PROMPT / extract`）
- 依赖任务: T9
- 完成标准:
...
- 影响文件: `simplecode/memory/auto_memory.py`（`_write_memories / _assign_section / _is_placeholder`）
- 依赖任务: T9, T10
- 完成标准:
...
- 影响文件: `simplecode/commands/handlers/memory.py`、`simplecode/commands/handlers/session.py`、`simplecode/commands/handlers/__init__.py`
- 依赖任务: T6, T7, T11
- 完成标准:
