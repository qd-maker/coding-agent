...
>
> 2026-07-17 验收：`ruff check simplecode tests`、`mypy simplecode`、`pytest -q` 均通过；
> 完整回归为 `319 passed, 1 skipped`。

## 1. 实现完整性

### 项目指令（simplecode/memory/instructions.py）
- [x] 模块常量 `MAX_INCLUDE_DEPTH = 5` 在 `/Users/codemelo/simplecode/simplecode/memory/instructions.py:5` 定义。
...
- [x] 函数 `load_instructions(project_root)` 在 `instructions.py:48-66` 实现：三层优先级（`<root>/SIMPLECODE.md` → `<root>/.simplecode/SIMPLECODE.md` → `~/.simplecode/SIMPLECODE.md`），每层跑 `process_includes`，多段用 `\n---\n` 拼接，无文件返回 `""`。
...
### 会话存档（simplecode/memory/session.py）
- [x] 模块常量 `SESSIONS_DIR = ".simplecode/sessions"` 在 `session.py:14` 定义。
...
### 自动记忆（simplecode/memory/auto_memory.py）
- [x] 模块常量 `USER_MEMORIES_RELPATH = ".simplecode/memories.md"` 在 `auto_memory.py:8` 定义。
- [x] 模块常量 `PROJECT_MEMORIES_RELPATH = ".simplecode/memories.md"` 在 `auto_memory.py:9` 定义。
...
### 包级 re-export（simplecode/memory/__init__.py）
- [x] `/Users/codemelo/simplecode/simplecode/memory/__init__.py` 集中 `from simplecode.memory.auto_memory import MemoryManager` / `from simplecode.memory.instructions import load_instructions, process_includes` / `from simplecode.memory.session import (ResumeResult, Session, SessionManager, SessionMeta, SessionRecord, build_time_gap_message, generate_session_summary, validate_message_chain)`。
- [x] `__all__` 与导入名一一对应，无遗漏。

### 对话注入（simplecode/conversation.py）
...
### 命令处理（simplecode/commands/handlers/）
- [x] `/memory` 命令处理在 `simplecode/commands/handlers/memory.py:6-46` 实现：`memory_manager is None` 时提示「记忆管理器未初始化」；空 / `list` 子命令打印 `get_display_text`；`clear` 调用 `MemoryManager.clear` 并提示「所有自动记忆已清空。」；`edit` 打印 user/project 路径；未知子命令打印 usage。
- [x] `MEMORY_COMMAND` 与 `handle_memory` 已迁移到 CH10 的统一 `Command/CommandContext` 架构，并由 `register_all_commands` 注册。
- [x] `/session` 命令处理在 `simplecode/commands/handlers/session.py` 实现：list / resume / new / delete 子命令；resume 后追加 `build_time_gap_message` 返回值；resume 切换 session/conversation 后复位 `ctx.agent._loop_count = 0`。

## 2. 接入完整性（必查，杜绝死代码）

- [x] `from simplecode.memory` 接入链可搜索到 `app.py` 启动装配、`agent.py` 类型依赖、`commands/handlers/session.py` 恢复命令和 `memory/__init__.py` re-export。
...
- [x] `grep -rn "MEMORY_EXTRACTION_INTERVAL\|_extract_memories\|_loop_count" /Users/codemelo/simplecode/simplecode --include="*.py" | grep -v "/tests/"` 命中 `agent.py` 主逻辑。
- [x] `grep -rn "load_instructions" /Users/codemelo/simplecode/simplecode --include="*.py" | grep -v "/tests/"` 命中 `app.py:634` 与 `memory/instructions.py / memory/__init__.py`。
- [x] `grep -rn "build_time_gap_message" /Users/codemelo/simplecode/simplecode --include="*.py" | grep -v "/tests/"` 命中 `commands/handlers/session.py` 与定义点。
- [x] 用户输入到本模块的路径可一句话描述：
...
- [x] `ruff check simplecode/memory/ simplecode/agent.py simplecode/conversation.py simplecode/commands/handlers/memory.py simplecode/commands/handlers/session.py` 无错误。
...
- [x] `python -c "from simplecode.memory import MemoryManager, load_instructions, process_includes, Session, SessionManager, SessionMeta, SessionRecord, ResumeResult, build_time_gap_message, generate_session_summary, validate_message_chain; print('ok')"` 打印 `ok`。

## 4. 端到端验证

- [x] 项目根放一份 `SIMPLECODE.md`，TUI 启动后的 Provider snapshot 能看到 `## 项目指令\n...` 段（`test_ch9_tui_loads_instructions_persists_and_exposes_commands`）。
- [x] `SIMPLECODE.md` 内写 `@include ./sub/style.md`（sub/style.md 在项目内）能展开；写 `@include ../../etc/passwd` 显示为 `<!-- @include blocked: path outside project -->`；写 `@include ./nonexistent.md` 显示为 `<!-- @include skipped: file not found -->`。
- [x] TUI 收发消息后退出，`.simplecode/sessions/` 出现一对 `session_*.jsonl` + `session_*.meta`；集成测试逐行 `json.loads` 并验证 user/assistant 记录。
