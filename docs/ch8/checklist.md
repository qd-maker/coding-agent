...
- [x] `SINGLE_RESULT_CHAR_LIMIT = 5_000` 在 `simplecode/context/manager.py:16` 定义。
...
- [x] `SUMMARY_OUTPUT_RESERVE = 20_000`、`AUTO_COMPACT_SAFETY_MARGIN = 13_000`、`MANUAL_COMPACT_SAFETY_MARGIN = 3_000`、`PERSISTED_TAG = "<persisted-output>"`、`SESSION_SUBDIR = ".simplecode/session/tool-results"` 在 `manager.py:24-30` 定义。
- [x] `ensure_session_dir(work_dir) -> Path` 在 `manager.py:132` 实现：创建并返回 `Path("<work_dir>/.simplecode/session/tool-results")`，`mkdir(parents=True, exist_ok=True)`。
...
- [x] `_EPHEMERAL = {"type": "ephemeral"}` 在 `simplecode/client.py:24` 定义。
...
### 1.6 `RecoveryState` 与恢复块（同样在 `simplecode/context/manager.py`）
...
- [x] `simplecode/context/__init__.py` re-export `RecoveryState / FileReadRecord / SkillInvocationRecord / build_recovery_attachment`。
...
- [x] `grep -rn "from simplecode.context" simplecode --include="*.py" | grep -v "context/"` 至少 1 处导入：
  - `simplecode/agent.py:15-27`（导入 `CompactCircuitBreaker / CompactEvent / ContentReplacementRecord / ContentReplacementState / append_replacement_records / apply_tool_result_budget / auto_compact / create_replacement_state / ensure_session_dir / load_replacement_records / reconstruct_replacement_state`）。
- [x] `grep -rn "apply_tool_result_budget\|auto_compact\|manual_compact\|clone_replacement_state" simplecode --include="*.py"` 命中：
  - `simplecode/agent.py:316`（`Agent.__init__` 初始化 `replacement_state`）
  - `simplecode/agent.py:510`（主循环调 `apply_tool_result_budget`）
  - `simplecode/agent.py:998`（另一主循环变体调 `apply_tool_result_budget`）
  - `simplecode/agent.py` 中 `manual_compact` 调 `auto_compact(..., manual=True)`
  - `simplecode/commands/handlers/compact.py`（`/compact` 命令调 `ctx.agent.manual_compact`）
  - `simplecode/tools/agent_tool.py:200-203`（fork 调 `clone_replacement_state` 注入子 Agent）
- [x] `grep -rn "RecoveryState\b\|recovery_state" simplecode --include="*.py"` 命中：
  - `simplecode/context/manager.py` 定义。
  - `simplecode/context/__init__.py` re-export。
  - `simplecode/agent.py`：`Agent.__init__` 初始化 + 三处 `auto_compact` kwarg + `_snapshot_for_recovery` 方法 + 两处 `_execute_*` 调用。
  - `simplecode/skills/executor.py`：`execute_inline` / `execute_fork` 各一处 `record_skill_invocation`。
- [x] 调用入口位于 `Agent.run` 的每轮 `for iteration` 循环内，并在
  `client.stream` 之前完成 Layer 1 提交与 Layer 2 判断。
- [x] 命令注册中心已更新: `COMPACT_COMMAND` 在 `simplecode/commands/handlers/__init__.py` 导出，由 registry 注册到 `/compact` + 别名 `/c`。
...
- [x] **死代码核查**：Token 估算、replacement state、Layer 1/2、恢复附件、熔断器与
  session helpers 全部在 `simplecode/context/__init__.py` 导出，并被 Agent、命令处理器或测试
  引用。

## 3. 编译与测试

- [x] `ruff check simplecode/context simplecode/commands/handlers/compact.py simplecode/client.py simplecode/agent.py simplecode/tools/agent_tool.py` 通过。
