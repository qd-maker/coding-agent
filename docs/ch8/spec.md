...
- F5: spill 文件 `persist_tool_result` 用 `os.open(O_WRONLY | O_CREAT | O_EXCL)` 写到 `<work_dir>/.simplecode/session/tool-results/<tool_use_id>.txt`，`FileExistsError` 静默吞掉（幂等）。
...
- F18: Fork 子 Agent 路径 `simplecode/tools/agent_tool.py` 创建 sub_agent 后判断 `p.subagent_type is None`（即真 fork）时 `sub_agent.replacement_state = clone_replacement_state(self._parent_agent.replacement_state)`。
...
- F20: 限额常量 `RECOVERY_FILE_LIMIT = 5` / `RECOVERY_TOKENS_PER_FILE = 5_000` / `RECOVERY_SKILLS_BUDGET = 25_000` / `RECOVERY_TOKENS_PER_SKILL = 5_000` / `_RECOVERY_CHARS_PER_TOKEN = 3.5` 在 `simplecode/context/manager.py` 顶部段定义。`_approx_tokens` 按 3.5 chars/token 折算；`_truncate_by_tokens` 按预算硬切并追加 `\n… (内容已截断)` 标记。
...
- N4: spill 写盘幂等：用 `O_CREAT | O_EXCL`，同 `tool_use_id` 重复运行写同一份内容，已存在则 `FileExistsError` 静默跳过；spill 文件路径稳定（`<work_dir>/.simplecode/session/tool-results/<tool_use_id>.txt`），不含时间戳。
...
- 核心模块结构 (`simplecode/context/manager.py`):
...
- Anthropic 客户端缓存断点（`simplecode/client.py`）:
...
- 与其他模块的交互:
  - 依赖 `simplecode.conversation`（`ConversationManager / Message / ToolResultBlock / ToolUseBlock` 与 `inject_environment / inject_long_term_memory / replace_history / serialize`）。
  - 依赖 `simplecode.tools.base`（`TextDelta / StreamEnd / StreamEvent` 收摘要 stream 事件）。
  - 被 `simplecode.agent.Agent`（主循环 + `manual_compact` + 另一主循环变体）、`simplecode.commands.handlers.compact`（`/compact` 命令）、`simplecode.tools.agent_tool`（fork clone）调用。
