...
```text
simplecode [--config PATH]
python -m simplecode [--config PATH]
```
...
system_prompt: You are Simple Code, a concise coding assistant.
...
- Layer 1：单条工具结果超过 5,000 字符，或本轮结果聚合超过 20,000 字符时，完整内容
  写入 `.simplecode/session/tool-results/<tool_use_id>.txt`，历史仅保留
...
- 发现顺序固定为 `<project>/SIMPLECODE.md`、`<project>/.simplecode/SIMPLECODE.md`、
  `~/.simplecode/SIMPLECODE.md`；高优先级内容排在前面，以 `\n---\n` 分隔。
...
- 用户级文件：`~/.simplecode/memories.md`；项目级文件：
  `<project>/.simplecode/memories.md`。
...
- 搜索顺序固定为 `<project>/.simplecode/skills`、`~/.simplecode/skills`、包内 builtins；
...
Agent 将工具结果写回会话后自动请求下一步，直到模型返回最终文本；每个用户回合最多运行 50 轮。达到上限时返回 `ErrorEvent`。TUI 仅展示工具摘要和耗时；超长结果保存到 `.simplecode/session/tool-results/`，预览和文件位置进入对话历史。
...
- `coordinator_mode=True` 时身份主体由 `simplecode.teams.coordinator` 提供，仍可追加环境、
...
- worktree 路径为 `<repo>/.simplecode/worktrees/<name>`；分支为
...
- 活动 session 原子写入 `.simplecode/worktree_session.json`；清空时文件内容为 `{}`。
...
- `teammate_mode` 接受 `"" / auto / in-process / tmux / iterm2`。显式 pane backend 不可用时返回
  错误，不回退到协程。
- pane CLI 入口为 `simplecode -p --work-dir <path> [--agent-type X] [--model X] <prompt>`，团队信息
  由 `SIMPLECODE_TEAM_NAME / SIMPLECODE_TEAMMATE_NAME / SIMPLECODE_MAILBOX_DIR` 传递。
...
## 16. CH16 稳妥交付 Contract

### EditFile

- 当工具挂了 `FileCache`：目标文件必须处于 `fresh` 状态，否则返回 `is_error=True`，磁盘不变。
- `missing` → `Error: file has not been read in this session: {path}. Call ReadFile first.`
- `stale` 或写入前再次变化 → `Error: file changed since last read: {path}. Call ReadFile again.`
- `WriteFile` 成功后缓存视为 fresh，允许立刻 `EditFile`。
- 没有 cache 时保持“精确唯一替换”旧行为。

### 工具结果配对

- 每个 assistant `tool_use.id` 必须对应恰好一条 user `tool_result.tool_use_id`。
- `ConversationManager.ensure_tool_result_pairing(reason)` 在发给模型前执行：
  - 缺结果：插入 `is_error=True` 的 synthetic 结果。
  - 孤儿结果 / 重复 id：丢弃多余的一边。
- 取消或异常时，未完成的调用补：
...
- `Agent.allow_permission_prompts` 默认 `True`。
- `run_to_completion` 期间视为无人值守：`ask` 转 `deny`，不 yield `PermissionRequest`。
- 拒绝文案：`Permission denied: non-interactive run cannot ask for confirmation.`
- `-p` 仍使用 `PermissionMode.DONT_ASK`；本条是第二道保险。

### 上下文超限

- `client.stream` 抛出 prompt-too-long 类错误时，强制 compact 后重试，最多 2 次。
- 成功发出 `RetryEvent("prompt_too_long compact N")`。
- `CancelledError` 不进入重试。

### 只读并发与 MCP 标注

- `Tool.concurrency_safe_for(arguments)` 决定分批；Bash 对 `is_safe_command(command)` 为 True。
- MCP `annotations.readOnlyHint=true` → `category="read"`，可并行，Plan 可用。
- MCP `annotations.destructiveHint=true` → `is_destructive=True`，不并行。
- MCP 描述超过 2048 字符截断。
