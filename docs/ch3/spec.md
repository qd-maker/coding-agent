...
- F17: 流式事件类型 `TextDelta` / `ToolCallStart` / `ToolCallDelta` / `ToolCallComplete` / `ThinkingDelta` / `ThinkingComplete` / `StreamEnd` 集中定义在 `simplecode/tools/base.py`，便于 LLM client 与 Agent Loop 共享类型。
...
- N3: 工具实现不允许依赖上层模块（agent / skills / teams），`simplecode/tools/` 处于底层；只能用 `simplecode.cache.FileCache`（typing-only）做可选注入。
...
- 与其他模块的交互:
 - 被依赖: `simplecode/agent.py`（取 schema、查工具、执行）、`simplecode/app.py`（创建并注册）、`simplecode/mcp/manager.py`（注册 MCP 工具 wrapper）、`simplecode/agents/tool_filter.py`（SubAgent 工具过滤复制 Registry）、`simplecode/skills/executor.py`（用 `allowedTools` 拷贝过滤的 Registry）、`simplecode/hooks/`（按 ToolName 覆盖）。
...
- 工具输出大结果存盘（spillover）：放到 ch08 `simplecode/context/`。
