...
- 核心数据结构（Python 类型）:
  - `MCPServerConfig`（`mewcode/config.py`，Pydantic model）：承载 `name / command / args / url / headers / env`，并校验传输二选一与名称字符集，提供 `is_stdio` property。
...
- 主流程（调用链）:
  - `mewcode/__main__.py` 启动 `MewCodeApp(config)`；App 从完整 `AppConfig` 读取 `config.mcp_servers`。
