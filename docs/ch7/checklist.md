...
- [x] `MCPServerConfig` 在 `simplecode/config.py` 实现，字段含 `name / command / args / url / headers / env / startup_timeout / tool_timeout`，`is_stdio == (command is not None)`。
...
- [x] `SimpleCodeApp` 从 `AppConfig.mcp_servers` 保存配置。
...
- [x] `uv run python -m compileall -q simplecode tests` 通过。
- [x] `uv run ruff check simplecode tests` 通过。
- [x] `uv run mypy simplecode` 通过。
...
- [x] `docs/api-contract.md`、`simplecode.yaml.example`、README 和章节索引已同步 MCP Contract。
