...
- 影响文件: `simplecode/config.py`（Pydantic `MCPServerConfig`、`is_stdio`、`resolve_env_vars`、`build_child_env`）
- 依赖任务: 无
...
- 影响文件: `simplecode/config.py`（`providers` / `mcp_servers` 列表模型、旧格式规范化、
...
- 影响文件: `simplecode/mcp/client.py:17-65`
- 依赖任务: T1
...
- 影响文件: `simplecode/mcp/client.py:86-113`
- 依赖任务: T3
...
- 影响文件: `simplecode/mcp/tool_wrapper.py:57-109`
- 依赖任务: T4
...
- 影响文件: `simplecode/mcp/tool_wrapper.py:12-54`
- 依赖任务: T5
...
- 影响文件: `simplecode/mcp/manager.py:13-70`
- 依赖任务: T5, T6
...
## T8: 暴露 `MCPManager` 出包
- 影响文件: `simplecode/mcp/__init__.py:1-5`
- 依赖任务: T7
- 完成标准: `__init__.py` 通过 `__all__ = ["MCPManager"]` 暴露，调用方写 `from simplecode.mcp import MCPManager` 即可。

## T9: 接入 Textual TUI 启动流程
- 影响文件: `simplecode/app.py:50`（import），`simplecode/app.py:514-525`（构造参数），`simplecode/app.py:537-538`（实例字段），`simplecode/app.py:810-811`（`on_mount` 派任务），`simplecode/app.py:1042-1044`（发消息前 await），`simplecode/app.py:1068-1070`（追加 system reminder），`simplecode/app.py:1496-1532`（`_init_mcp`），`simplecode/app.py:1534-1544`（`_shutdown_mcp`）
- 依赖任务: T8
