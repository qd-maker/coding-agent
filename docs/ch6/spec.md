...
- N1: 危险命令模式必须硬编码进 `simplecode/permissions/dangerous.py` 的 `_DANGEROUS_PATTERNS`，不依赖外部下载或环境变量注入，避免被攻击者绕过。
...
- 调用链:
 - `SimpleCodeApp._build_agent` 装配 → 构造 `PermissionChecker` + `PathSandbox` + `RuleEngine` → 传给 `Agent`。
...
- 与其他模块的交互:
 - 依赖 `simplecode.tools.base.Tool`（`name`、`category` 字段）。
 - 依赖 `PyYAML` 做规则文件序列化。
 - 被 `simplecode.agent.Agent`、`simplecode.app.SimpleCodeApp`、`simplecode.permission_dialog.InlinePermissionWidget` 直接使用；子 Agent（`simplecode.agents.fork`）继承父 Agent 的 `permission_checker`。
