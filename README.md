...
`Python` · `MCP` · `ReAct` · `Plan Mode` · `Skill` · `Multi-Agent` · `Anthropic / OpenAI`

Simple Code 使用 Python + Textual 实现，将模型流式输出、结构化工具调用、Agent Loop、
上下文回灌、System Prompt、权限控制和跨会话持久化串成一条可运行、可观察、可测试的执行链路。

以 **ReAct**（推理—行动—观察循环）与 **Plan Mode**（先只读理解、再输出实施计划）双模式驱动
LLM 自主完成编程任务；整体按**交互、引擎、工具、记忆、安全**五层分层，兼容 Anthropic / OpenAI
双协议，支持 MCP 工具扩展、Skill 技能包、跨会话记忆与多 Agent 并行协作。
...
![Simple Code TUI 入口界面](docs/assets/tui-overview.png)
...
Simple Code 按职责分为五层，每层只依赖下层的稳定 Contract，任一层可独立替换与测试：

```text
┌─────────────────────────────────────────────────────────────┐
│  交互层  Textual TUI · Slash Command · HITL 内联权限交互       │  app.py / commands/
├─────────────────────────────────────────────────────────────┤
│  引擎层  Agent Loop · ReAct/Plan 双模式 · Provider Adapter    │  agent.py / client.py
│          ConversationManager · SubAgent · AgentTeam           │  conversation.py / agents/ / teams/
├─────────────────────────────────────────────────────────────┤
│  工具层  Tool Contract · 内置工具 · MCP 延迟加载 · Skill      │  tools/ / mcp/ / skills/ / hooks/
├─────────────────────────────────────────────────────────────┤
│  记忆层  分层项目指令 · JSONL 会话存档 · 自动记忆提取         │  memory/ / context/
├─────────────────────────────────────────────────────────────┤
│  安全层  危险命令检测 · 路径沙箱 · 规则引擎 · 权限模式 · HITL │  permissions/
└─────────────────────────────────────────────────────────────┘
```

- **交互层**：终端原生 TUI 消费引擎事件流，展示流式回答、思考、工具状态；Slash Command 优先拦截输入，权限确认走内联 HITL。
- **引擎层**：Agent Loop 驱动「模型 → 工具 → 回灌 → 下一轮」循环，管理停止条件、并发批次与上下文预算；Provider Adapter 把不同厂商 SDK 事件归一成统一 `StreamEvent`；SubAgent / AgentTeam 支持上下文 Fork 与多 Agent 并行。
- **工具层**：统一 `Tool` Contract + Pydantic 校验，内置文件/搜索/Bash 工具，MCP 外部工具延迟加载，Skill 技能包与 Hook 生命周期自动化。
- **记忆层**：三层 `SIMPLECODE.md` 项目指令、崩溃安全的 JSONL 会话存档、两层上下文压缩，以及每轮结束后异步提取的双层自动记忆。
- **安全层**：在模型之外做程序化拦截——危险命令检测、路径沙箱、YAML 规则引擎、权限模式矩阵、人工确认，任一层拒绝即终止操作。
...
| 指令、会话与自动记忆 | ✅ | 三层 `SIMPLECODE.md`、安全 `@include`、崩溃安全 JSONL、链路校验、双层 `memories.md`、`/session`、`/memory` |
...
- **MCP 工具延迟加载**：MCP 工具注册时只挂名称不注入完整 schema，LLM 通过 `ToolSearch` 按需发现后再加载完整定义。百级工具场景下工具描述的 Token 占用显著降低，避免上下文窗口被工具定义挤占。
- **多 LLM 协议适配**：将 Anthropic Messages 与 OpenAI Responses 两套流式响应协议统一为同一套 `StreamEvent` 事件接口，Agent 不依赖具体协议，新增供应商只需适配一个 Adapter。
- **两层上下文压缩**：渐进式的两层压缩策略（大结果落盘 + 预览回灌、结构化摘要）自动对齐 Function Calling 的 tool_use / tool_result 配对约束，支持数小时连续会话不丢失上下文。
- **五层权限拦截**：命令拦截、路径沙箱、规则引擎、权限模式、人工确认五层串联，任一层拒绝即终止；YOLO 全自动模式下危险命令检测与路径沙箱两层仍不可绕过。
- **自动记忆提取**：每轮对话结束后异步调用 LLM 回顾内容，将「用户偏好、纠正反馈、项目知识、参考资料」四类记忆分类持久化到用户级 / 项目级文件，跨会话累积、同类纠正只需一次、新会话自动继承项目上下文。
- **多 Agent 协作**：Coordinator 只负责任务拆分与结果汇总，不参与编码；子任务分派给多个 Agent，基于 Git Worktree 做文件级隔离避免编辑冲突，突破单 Agent 上下文窗口瓶颈。
...
`.simplecode/session/tool-results/`，只把结果预览和文件位置回灌给模型；声明为并发安全的相邻只读工具可
并行执行。
...
安装完成后，任意目录直接输入 `simplecode` 即可启动。
...
```powershell
Copy-Item simplecode.yaml.example simplecode.yaml
...
配置发现顺序：当前目录的 `simplecode.yaml`，然后是 `~/.simplecode/config.yaml`。也可以使用
`--config` 显式指定路径。
...
simplecode -p "解释这个项目的入口"
```

### 4. 可选：项目指令与持久记忆

在项目根目录创建 `SIMPLECODE.md`，可记录技术栈、编码规范和项目注意事项：
...
Simple Code 按 `<project>/SIMPLECODE.md` → `<project>/.simplecode/SIMPLECODE.md` →
`~/.simplecode/SIMPLECODE.md` 的顺序加载。`@include` 最多递归 5 层，且不能跳出项目目录。

会话消息追加保存到 `<project>/.simplecode/sessions/*.jsonl`，摘要和消息数保存在同名 `.meta`
文件。自动记忆分开写入 `~/.simplecode/memories.md`（用户偏好、纠正反馈）与
`<project>/.simplecode/memories.md`（项目知识、参考资料）。这些运行时文件已被 `.gitignore`
忽略。
...
Simple Code 目前的应对方式是：统一事件和数据 Contract、结构化工具结果、明确停止条件、程序化
权限层、分层上下文预算，以及以 checklist 和自动测试验证完成声明。

## 验证

```powershell
uv run python -m compileall -q simplecode tests
uv run ruff check simplecode tests
uv run mypy simplecode
uv run pytest -q
```
...
├── memory/               # SIMPLECODE.md、JSONL 会话与自动记忆
...
- `simplecode --resume`：进程中断后恢复上次活动的 worktree session。
